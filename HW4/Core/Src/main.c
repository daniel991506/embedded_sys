...
/* USER CODE BEGIN PV */
int32_t accel_data[3] = {0};    // 3-axis accelerometer values
uint16_t sampling_frequency = 1000;  // Default frequency (in ms)
/* USER CODE END PV */
...
int main(void)
{
  ...
  freqQueueHandle = osMessageQueueNew (16, sizeof(uint16_t), &freqQueue_attributes);
  ...
  TASK_BLEHandle = osThreadNew(StartDefaultTask, NULL, &TASK_BLE_attributes);

  /* creation of TASK_ACC */
  TASK_ACCHandle = osThreadNew(StartTask02, NULL, &TASK_ACC_attributes);
